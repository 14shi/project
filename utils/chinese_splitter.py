...
"""
据给定的分隔符（ separator ）对输入的文本（ text ）进行分割，
并根据 keep_separator 参数决定是否保留分隔符。
最终返回一个分割后的字符串列表，且过滤掉空字符串。
"""
...
if __name__ == "__main__":
    text_splitter = ChineseRecursiveTextSplitter(
        keep_separator=True, is_separator_regex=True, chunk_size=128, chunk_overlap=0
    )
    ls = [
